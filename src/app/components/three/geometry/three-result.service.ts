...
export class ThreeResultService {
  ...
  private createLines(circleData, circleNode): THREE.Object3D {
    ...
    const curve_s = new THREE.Vector2(
      circleNode[0].x - circleData.x,
      circleNode[0].y - circleData.y
    );
    const curve_d = new THREE.Vector2(
      circleNode[circleNode.length - 1].x - circleData.x,
    ...
    // 地表面から中心点までの直線を描く
    const points = [
      new THREE.Vector3(curve_s.x, curve_s.y, 0),
